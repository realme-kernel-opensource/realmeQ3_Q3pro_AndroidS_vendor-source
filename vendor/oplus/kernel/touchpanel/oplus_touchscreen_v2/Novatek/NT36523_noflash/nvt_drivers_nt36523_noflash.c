...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	case PEN_DETECT:
		gesture->gesture_type  = chip_info->ts->is_pen_attracted?UNKNOWN_GESTURE:PENDETECT;
		break;

	default:
		gesture->gesture_type = UNKNOWN_GESTURE;
		break;
	}
...
MEM_ALLOC_ERR:
	...
	if (chip_info->ts->pen_support) {
		return (noise_p_result + noise_n_result +
				pen_tip_x_noise_p_result + pen_tip_x_noise_n_result +
...
static int nvt_tp_probe(struct spi_device *client)
{
	...
	ts->tp_suspend_order = TP_PRE_LCD_SUSPEND;
