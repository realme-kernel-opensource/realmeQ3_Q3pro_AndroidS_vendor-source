...
static void zeroflash_do_romboot_firmware_download(void)
{
	...
	if(tcm_hcd->tp_fw_update_first) {
		tcm_hcd->tp_fw_update_first = false;
			TPD_INFO("000000000000Prepare ROMBOOT firmware download\n");
		return;
	}

	TPD_INFO("Prepare ROMBOOT firmware download\n");
	...
	if (retval < 0) {
		TPD_INFO("Failed to allocate memory for application firmware\n");
		UNLOCK_BUFFER(g_zeroflash_hcd->out);
		goto exit;
	}
