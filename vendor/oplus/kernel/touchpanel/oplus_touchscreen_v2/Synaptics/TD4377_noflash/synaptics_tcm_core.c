...
extern int tp_reset_config(bool enable);
				  ...
				  unsigned int timeout)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		TPD_INFO("%s:[%d] buf[0]-0x%02x buf[1]-0x%02x buf[2]-0x%02x buf[3]-0x%02x \n", __func__, idx, \
...
void tp_wait_hdl_finished(void)
{
	...
	TPD_INFO("tp_wait_hdl_finished Begin\n");
	do {
		if (retry_cnt) {
			msleep(100);
		}
	...
	TPD_INFO("tp_wait_hdl_finished End\n");
}
