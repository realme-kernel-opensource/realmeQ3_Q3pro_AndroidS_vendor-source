...
static int synaptics_before_black_screen_autotest(struct seq_file *s,
...
static int synaptics_doing_black_screen_autotest(struct seq_file *s,
		struct touchpanel_data *ts, struct auto_testdata *p_nvt_testdata)
{
	int error_count = 0;
	int ret = 0;
	...
	p_test_item_info = get_test_item_info(p_nvt_testdata->fw, TYPE_TEST6);

	if (!syna_test_ops->syna_black_screen_test_noise) {
	...
	p_test_item_info = get_test_item_info(p_nvt_testdata->fw, TYPE_TEST7);
	if (!syna_test_ops->syna_black_screen_test_dynamic) {
		TPD_INFO("not support nvt_test_ops->syna_black_screen_test_dynamic callback\n");

	} else {
		ret = syna_test_ops->syna_black_screen_test_dynamic(s, ts->chip_data, p_nvt_testdata,
						       p_test_item_info);

		if (ret < 0) {
			TPD_INFO("test [syna_black_screen_test_dynamic] failed! ret is %d\n", ret);
			error_count++;
		}
	}
			      ...
			      struct touchpanel_data *ts)
{
	...
	TPD_INFO("%s + \n", __func__);
	ret = synaptics_before_black_screen_autotest(NULL, ts, &nvt_testdata);
	...
	msleep(5);
	error_count += synaptics_doing_black_screen_autotest(NULL, ts, &nvt_testdata);
...
EXPORT_SYMBOL(synaptics_black_screen_test);
