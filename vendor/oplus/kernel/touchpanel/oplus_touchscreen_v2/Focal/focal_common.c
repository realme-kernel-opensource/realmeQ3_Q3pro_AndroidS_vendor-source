...
static int hex_to_str(char *hex, int iHexLen, char *ch, int *iChLen)
{
	...
	ch[ch_len] = '\0';
	*iChLen = ch_len;
	TPD_INFO("ch_len: %d, iChLen: %d in function:%s!!\n", ch_len, *iChLen,
		 __func__);
	return 0;
}
...
static ssize_t focal_hw_reset_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	ssize_t count = 0;
	struct touchpanel_data *ts = dev_get_drvdata(dev);
	struct focal_debug_func *focal_debug_ops = (struct focal_debug_func *)
			ts->private_data;
...
static ssize_t focal_irq_store(struct device *dev,
...
static ssize_t focal_fw_version_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
...
static ssize_t focal_rw_reg_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int count;
...
static ssize_t focal_rw_reg_store(struct device *dev,
				  struct device_attribute *attr, const char *buf, size_t count)
{
	struct touchpanel_data *ts = dev_get_drvdata(dev);
	struct focal_debug_func *focal_debug_ops = (struct focal_debug_func *)
...
static ssize_t focal_esdcheck_store(struct device *dev,
				    struct device_attribute *attr, const char *buf, size_t count)
{
	struct touchpanel_data *ts = dev_get_drvdata(dev);
...
static ssize_t focal_esdcheck_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int count = 0;
	struct touchpanel_data *ts = dev_get_drvdata(dev);
	...
	if (ts->esd_handle_support) {
		if (focal_debug_ops && focal_debug_ops->get_esd_check_flag) {
			count = sprintf(buf, "esd_running_flag: %d,  esd_check_need_stop: %d\n", \
					...
					focal_debug_ops->get_esd_check_flag(ts->chip_data));
...
static ssize_t focal_dump_reg_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	char tmp[256];
	int count = 0;
...
static DEVICE_ATTR(fts_fw_version, S_IRUGO | S_IWUSR, focal_fw_version_show,
		   NULL);
...
static DEVICE_ATTR(fts_rw_reg, S_IRUGO | S_IWUSR, focal_rw_reg_show,
		   focal_rw_reg_store);
...
static DEVICE_ATTR(fts_esd_check, S_IRUGO | S_IWUSR, focal_esdcheck_show,
		   focal_esdcheck_store);
...
static ssize_t focal_debug_write(struct file *filp, const char __user *buff,
...
static ssize_t focal_debug_read(struct file *filp, char __user *buff,
				size_t count, loff_t *ppos)
{
	int ret = 0;
...
int focal_create_apk_debug_channel(struct touchpanel_data *ts)
{
	...
	focal_proc_entry = proc_create_data("grip_handle", 0222, ts->prEntry_tp,
					    &proc_grip_control_ops, ts);
...
void ft_limit_read_std(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	if ((ph->magic1 != LIMIT_HEADER_MAGIC_1)
			|| (ph->magic2 != LIMIT_HEADER_MAGIC_2)) {
...
/**** focal do auto test ****/
static int focal_test_item(struct seq_file *s, struct touchpanel_data *ts,
			   struct auto_testdata *p_focal_testdata)
{
	...
	if (!fts_test_ops->auto_test_preoperation) {
		...
		ret = fts_test_ops->auto_test_preoperation(s, ts->chip_data, p_focal_testdata,
				p_test_item_info);
			       ...
			       struct auto_testdata *p_focal_testdata)
{
	...
	p_focal_testdata->fp = ts->com_test_data.result_data;
	p_focal_testdata->length = ts->com_test_data.result_max_len;
...
int focal_auto_test(struct seq_file *s,  struct touchpanel_data *ts)
{
	...
	error_count += focal_test_item(s, ts, &focal_testdata);

END:
	seq_printf(s, "imageid = 0x%llx, deviceid = 0x%llx\n", focal_testdata.tp_fw,
		   focal_testdata.dev_tp_fw);
